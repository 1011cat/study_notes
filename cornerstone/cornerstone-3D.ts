...
export default class CornerStone extends Vue {
  showMultiCell = false
  imageIdURLTemp = "http://shenzhou.10.6.120.131.sslip.io/imageServer/Home/ImageService?CommandType=GetImage&contentType=application%2Fdicom&OBJECTUID="
  allImageInfo = null
  imageIds = null
  ...
  async initCornerstone() {
    ...
    this.renderingEngine = new RenderingEngine('dom0');
    const viewportId = 'dom0';
    const viewportInput = {
      viewportId,
      type: ViewportType.STACK,
      element: dom0 as HTMLDivElement,
    };
  ...
  async start() {
    ...
    // dom.addEventListener("cornerstoneimagerendered", (e) => {
  ...
  displayFistImage(dom, callback) {
    const viewport = <Types.IStackViewport>this.renderingEngine.getViewport('dom0');
    viewport.setStack([this.imageIds[0]]).then(() => {
      viewport.render()
      console.log(111111111111, viewport, cornerstone, cornerstoneDICOMImageLoader, cornerstoneTools)
      // const image = viewport.csImage;
    })
    // cornerstone.loadAndCacheImage(this.imageIds[0]).then(image => {
    //   console.log(1111111111111113333333333, image, JSON.stringify(image))
  ...
  playVideo(e) {
    ...
    // }
    // console.log(888888888888, cornerstoneWADOImageLoader, cornerstone, cornerstone.imageCache.getCacheInfo())
  }
  ...
  cacheAllImages(max = 5, cb = null){
    ...
    //         return
    //       }
    //       cornerstone.loadAndCacheImage(this.imageIds[index]).then(image => {
    //         index++
    //         const imageId = this.imageIds[index]
    //         setTimeout(() => {
    //           cornerstoneWADOImageLoader.webWorkerManager.terminate();
    //           cornerstoneWADOImageLoader.wadouri.dataSetCacheManager.purge();
    //           // cornerstoneWADOImageLoader.wadouri.metaDataManager.remove(imageId);
    //         }, 10);
    //         run()
    //       })
    //     }
